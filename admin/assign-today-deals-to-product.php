<?php
	...
	if(isset($_POST['submit']))
	{
	  ...
	  //	$offerprice = $_POST['offerprice'];
		...
		$sqlDeal =mysqli_query($conn, "select start_date,end_date,start_time,end_time from todaydeals where today_deal_id = '$deals'");
		...
		for($i = 0;$i < count($product); $i++){
		   	$insertuser = "INSERT INTO `assign_todays_deals`(`deal_id`,`product_id`,`start_date`,`end_date`,`start_time`,`end_time`,`offer_price`,`start_date_str`,`end_date_str`)VALUES ('".$deals."','".$product[$i]."','".$fetchDeal['start_date']."','".$fetchDeal['end_date']."','".$fetchDeal['start_time']."','".$fetchDeal['end_time']."','".$_POST['offerprice'.$product[$i]]."','$startstr','$endstr')";  
			$insertuser1=mysqli_query($conn,$insertuser);
			
	 $update = "update product set offer_price = '".$_POST['offerprice'.$product[$i]]."',startDate = '".$fetchDeal['start_date']."',start_time='".$fetchDeal['start_time']."',endDate = '".$fetchDeal['end_date']."',end_time='".$fetchDeal['end_time']."',deal_id = '".$deals."',`start_date_pstr`='".$startstr."',`end_date_pstr`='".$endstr."' where prod_id = '".$product[$i]."'";
	...
	//	$insertuser1=mysqli_query($conn,$insertuser);
			...
			if($insertuser1)
			{
				...
				echo '<script>window.location.href="assign-today-deals-to-product.php?sucmsg=Deal Assigned successfully"</script>';

			
			}
			else{
			
			$msg1="Assigned today Deal not Added";
		  }
		}
...
<div class="content-body">
    <div class="container-fluid">
		<div class="row">
	        <h2 class="mb-3 me-auto">Add Slider To Products</h2>
            <?php
                            ...
                            <form role="form" method="post" action="" enctype="multipart/form-data" id="myform">
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        <label class="form-label">Sliders<span style="color:red;font-size:12px"></span></label>
                                        <select class="default-select form-control wide">
    										<?php    while($rowS= mysqli_fetch_assoc($sqlFol1)) 
                							{
                							?>
                							
                                             <option value="<?php echo $rowS['today_deal_id']; ?>"><?php echo $rowS['deal_title']; ?></option>
                							<?php
                							}
                							?>
    									</select>
                                    ...
                                    <div class="col-md-12 mb-3">
            					        ...
            					        <div id="checkboxes" class="">
                                        	  ...
                                        	  ?>
                                        	    <div class="row p-3" style="margin-top:20px; margin-bottom:20px;">
                                            	    <div class="col-md-8">
                                                        <label for="one<?php echo $prorow['prod_id'];?>"></label>
                                                        <input name="product[]" value="<?php echo $prorow['prod_id'];?>"  type="checkbox" id="one<?php echo $prorow['prod_id'];?>" /><?php echo $prorow['prod_title'];?> ( <?php echo $prorow['sub_category_name'];?> )
                                            	    </div>
                                            	    <div class="col-md-2">
                                                        <input type="text" name="price" class="form-control" value="<?php echo $prorow['sale_price']?>" readonly/>
                                            		</div>
                                            		<div class="col-md-2">
