...
//$sqlFol1=mysqli_query($conn, "select * from deal where end_date_str >= '$dat'");
	...
	/*	for($k = 0;$k < count($product); $k++){
		...
		for($i = 0;$i < count($product); $i++){
		    $offerprice = 0;
		   	$insertuser = "INSERT INTO `assigned_deals_to_product`(`deal_id`,`product_id`,`start_date`,`end_date`,`start_time`,`end_time`,`offer_price`,`start_date_str`,`end_date_str`)
		   	        VALUES ('".$deals."','".$product[$i]."','".$fetchDeal['start_date']."','".$fetchDeal['end_date']."','".$fetchDeal['start_time']."','".$fetchDeal['end_time']."','".$offerprice."','$startstr','$endstr')"; 
	...
	//	$insertuser1=mysqli_query($conn,$insertuser);
			...
			if($insertuser1)
			{
				echo '<script>window.location.href="assign-deals-to-product.php?sucmsg=Slider Assigned successfully"</script>';
			}
			else
			{
...
<div class="content-body">
    <div class="container-fluid">
		<div class="row">
	        <h2 class="mb-3 me-auto">Add Sliders To Products</h2>
	        <?php
                            ...
                            <form role="form" method="post" action="" enctype="multipart/form-data" id="myform">
                                <div class="row">
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <select class="default-select form-control wide" id="categories" name="deals">
                                            <option selected disabled>Select Slider</option>
    									<?php
$valu = array();
 while($rowS= mysqli_fetch_assoc($sqlFol1)) 
{
?>
 <option value="<?php echo $rowS['deal_id']; ?>"><?php echo $rowS['deal_title']; ?></option>
 
<?php
$deal_id=$rowS['deal_id'];

$sql=mysqli_query($conn,"select * from assigned_deals_to_product where deal_id= $deal_id");
$checkarr=array();
while($result=mysqli_fetch_assoc($sql)){
 $checkarr[]=$result['product_id'];  
}
...
?>
						...
						<select class="form-control">
                    					 ...
                    					 <div id="checkboxes" class="custom-dropdown col-md-12">
                    					  <?php 
                          ...
                          <label for="one<?php echo $prorow['prod_id'];?>">
                            <input name="product[]" value="<?php echo $prorow['prod_id'];?>" class="mycheckboxes" type="checkbox" id="<?php echo $prorow['prod_id'];?>" /><?php echo $prorow['prod_title'];?> ( <?php echo $prorow['sub_category_name'];?> )</label>
                    	   <label>
                    	   </div>
                           </div>		
...
$("#categories").change(function() {
   var arr = <?php echo json_encode($valu,JSON_HEX_APOS) ?>;
   var id = $( this ).val();
   var val;
