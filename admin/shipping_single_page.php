...
$fetchsingleorder=mysqli_query($conn, "SELECT seller.*, order_product.* FROM seller LEFT JOIN order_product ON order_product.`seller_id`=seller.`seller_id`
                                       where order_product.`booking_id`='".$_GET['orderid']."' and order_product.`status`=1");
        				   ...
        				   ?>
							...
							<p>
							    Payment Status : <?php if($single['payment_status']==0) { ?>
									        	...
									        	<?php } elseif($single['payment_status']==1){ ?>
            ...
            <div class="col-12">
                            ...
                            <table class="table table-striped">
								...
								<td>
								  ...
								  while($rowimg=mysqli_fetch_assoc($img)){
									?>
								<img src="<?php echo $base_url; echo $rowimg['image']?>" class=" thumbnail" style="width:100px ; height:100px;" >
								<?php } ?>
								</td>
								<td>
								...
								<?php if( $rowsingleorder['availability']=='no'){?>
									<td>  <button type="button" class="btn btn-change" onclick="myFunction('<?php echo $rowsingleorder['orderid']?>')"><i class="fa fa-edit"></i>Change Status</button>
    			...
    			?>
    			 <td><?php echo $rowsingleorder['shop_name'];?>&nbsp;,&nbsp;<?php echo $rowsingleorder['location']; ?>&nbsp;,&nbsp;<?php echo $rowproduct11['cname']?>&nbsp;,&nbsp;<?php echo $rowproduct11['sname']?>&nbsp;,&nbsp;<?php echo $rowproduct11['cuname']?></td>
								...
								echo $tax_price=(($total1 * $fetchtax['tax'])/100);
								...
								<!--<td><strong>AED .<?php echo ($total1-$single['discount'])+$single['shipping_charge']?></strong> </td>-->
